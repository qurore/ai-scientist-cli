...
description: Stage 2 of the AI Scientist pipeline — run the agentic experiment loop (a Codex-native version of AI-Scientist-v2's best-first tree search) to implement, execute, debug, and improve experiment code across preliminary → tuning → research → ablation stages, recording a journal, metrics, and plots. Use after an idea exists and the user wants to actually run experiments.
---

# Stage 2 — Experiments (agentic tree search, Codex-native)
...
```
Requires `torch`+`psutil` (see `scripts/setup.sh`). In this collaborative repository,
the shared bridge routes agent LLM calls through Claude Code. Use it only if the user
explicitly wants upstream parity or cross-agent execution.
